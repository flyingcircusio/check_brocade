...
def main():
    parser = argparse.ArgumentParser(
        description='Check status of Brocade VDX cluster.')
    parser.add_argument('address')
    parser.add_argument('rbridge')
    parser.add_argument('username')
    parser.add_argument('password')
    args = parser.parse_args()

    cluster_address = args.address
    rbridge_id = args.rbridge

    c = requests.Session()
    c.auth = (args.username, args.password)

    r = c.post('http://{}/rest/operational-state/show-vcs'.format(cluster_address),
               data='<show-vcs></show-vcs>').content
    r = ElementTree.fromstring(r)
    for rbridge in r.findall('.//vcs:vcs-node-info', ns):
        this_id = rbridge.find('./vcs:node-rbridge-id', ns).text
        if this_id == rbridge_id:
            host = rbridge.find('./vcs:node-switchname', ns).text
            break
    else:
        raise KeyError("Unknown rbridge: {}".format(rbridge_id))
    ...
    r = c.post('http://{}/rest/operational-state/show-clock'.format(cluster_address),
               data='<show-clock><rbridge-id>{}</rbridge-id></show-clock>'.format(rbridge_id)).content
    ...
    time_state = ('OK' if time_state == OK
                  else 'WARN' if time_state == WARN
                  else 'CRIT' if time_state == CRIT
                  else '?')

    system.add_row(['Current time', time.strftime('%Y-%m-%d %H:%M:%S'),
                   time_state])

    r = c.post('http://{}/rest/operational-state/get-last-config-update-time'.
               format(cluster_address),
               data='<get-last-config-update-time></get-last-config-update-time>'
               ).content
    r = ElementTree.fromstring(r)
    timestamp = int(r.find('.//vcs:last-config-update-time', ns).text)
    last_change = datetime.datetime.fromtimestamp(
        timestamp, tz).astimezone(LOCAL_TIME)
    system.add_row(['Last config change',
                    last_change.strftime('%Y-%m-%d %H:%M:%S'), ''])

    r = c.post('http://{}/rest/operational-state/get-system-uptime'.
               format(cluster_address), data='<get-system-uptime><rbridge-id>{}</rbridge-id></get-system-uptime>'.format(rbridge_id)
               ).content
    r = ElementTree.fromstring(r)
    ...
    r = c.post('http://{}/rest/operational-state/show-system-monitor'.format(
        cluster_address), data='<show-system-monitor><rbridge-id>{}</rbridge-id></show-system-monitor>'.format(rbridge_id)).content
    r = ElementTree.fromstring(r)
    ...
    disconnected_nodes = int(
        r.find('.//vcs:nodes-disconnected-from-cluster', ns).text)
    ...
    vcs.add_row(['Disconnected Nodes',
                 disconnected_nodes, 'CRIT' if disconnected_nodes else 'OK'])
    ...
    rbridges.field_names = ['RBridge', 'Name', 'Serial', 'Status',
                            'Principal', 'Co-Ordinator', 'Firmware', 'Fabric']
    rbridges.align = 'l'
    ...
    for rbridge in r.findall('.//vcs:vcs-node-info', ns):
        ...
        rbridges.add_row(
            [rbridge_id, name, serial,
             '{}, {}, {}'.format(status, status2, status3),
             '*****' if is_principal else '',
             '*****' if is_coordinator else '', firmware, fabric_state])
    ...
    r = c.post('http://{}/rest/operational-state/show-raslog'.format(cluster_address),
               data='<show-raslog><rbridge-id>{}</rbridge-id></show-raslog>'.format(rbridge_id)).content
    r = ElementTree.fromstring(r)

    for entry in r.findall('.//raslog:raslog-entries', ns):
        time = entry.find('./raslog:date-and-time-info', ns).text
        time = datetime.datetime.strptime(time, '%Y/%m/%d-%H:%M:%S:%f').replace(tzinfo=pytz.UTC)
    ...
    for entry in r.findall('.//raslog:raslog-entries', ns):
        time = entry.find('./raslog:date-and-time-info', ns).text
        time = datetime.datetime.strptime(time, '%Y/%m/%d-%H:%M:%S:%f').replace(tzinfo=pytz.UTC)
        time = time.astimezone(LOCAL_TIME).strftime('%Y-%m-%d %H:%M:%S')
        severity = entry.find('./raslog:severity', ns).text
